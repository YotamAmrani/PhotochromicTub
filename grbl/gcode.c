...
uint8_t gc_execute_line(char *line)
{
  ...
  */
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    ...
    bit_false(value_words,(bit(WORD_N)|bit(WORD_F)| bit(WORD_S))); // EDITTED by yoyo - adding WORS_S
  } else {
  ...
  */
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) { // EDITTED by yoyo - Adding GROUP_M7 support spindle turn to jog mode 
    // Only distance and unit modal commands and G53 absolute override command are allowed.
    // NOTE: Feed rate word and axis word checks have already been performed in STEP 3.
    if (command_words & ~(bit(MODAL_GROUP_M7)|bit(MODAL_GROUP_G3) | bit(MODAL_GROUP_G6 | bit(MODAL_GROUP_G0))) ) { FAIL(STATUS_INVALID_JOG_COMMAND) };
